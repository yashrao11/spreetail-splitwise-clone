...
export default function GroupLedger({
  ...
  return (
    <div className="border-2 border-slate-300 bg-white p-6">
      <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2 border-b border-slate-300 pb-3">
      ...
      {initialFeed.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center p-8 sm:p-16 border border-dashed border-slate-300 bg-slate-50">
          <Receipt className="w-12 h-12 text-slate-400 mb-4" />
          <h3 className="font-bold text-slate-800 text-sm">No activity logged yet</h3>
        ...
        <div className="space-y-4">
          {initialFeed.map((item) => {
            ...
            return (
              <div
                key={`expense-${item.id}`}
                className="p-4 border border-slate-300 bg-white hover:bg-slate-50/30 transition-all duration-150 flex flex-col gap-3 relative"
              >
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500 mt-1 font-medium">
                      <span>
                        Paid by <strong className="text-slate-700">{payerName}</strong>
                      </span>
                  ...
                  <div className="flex items-center gap-3 shrink-0 text-right">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-450 text-slate-400 mt-1 font-semibold flex items-center justify-end gap-1">
                        <Calendar className="w-3 h-3" />
                        <span suppressHydrationWarning>
                          {formatDateTime(item.createdAt)}
                        </span>
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="p-1 border border-transparent hover:border-slate-300 bg-transparent hover:bg-slate-50 text-slate-400 hover:text-slate-700 cursor-pointer"
                      >
                        {deletingId === item.id ? (
                      ...
                      {activeMenuId === item.id && (
                        <div className="absolute right-0 mt-1 w-28 bg-white border border-slate-300 py-1 z-15 select-none shadow-sm animate-in fade-in duration-100">
                          <button
                            onClick={() => {
                              setEditingExpense(item);
                              setActiveMenuId(null);
                            }}
                            className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-slate-750 text-slate-700 hover:bg-slate-100 text-left font-semibold cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-[#ff652f] hover:bg-red-50 text-left font-semibold cursor-pointer"
                          >
                ...
                <div className="bg-slate-50 border border-slate-300 p-3 mt-1">
                  <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
                    Contributors & Splits Breakdown
                  </span>
                  
                  {isMultiPaid && (
                    <div className="mb-2.5 pb-2 border-b border-slate-300 text-[11px] text-slate-500 space-y-1">
                      <span className="font-semibold text-slate-600 block">Payments Log:</span>
                      {item.payments?.map((payment) => (
                        <span key={payment.id} className="inline-block bg-slate-200 border border-slate-300 px-1.5 py-0.5 mr-1.5 font-medium">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 border-b border-slate-300 pb-3 mb-2">
                    {item.splits?.map((split) => {
                      ...
                      return (
                        <div key={split.id} className="flex justify-between items-center text-xs">
                          <span className="text-slate-655 text-slate-600 truncate mr-2">
