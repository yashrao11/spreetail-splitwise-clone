...
import { Plus, X, Loader2, DollarSign, Calculator, AlertCircle } from 'lucide-react';
...
export default function AddExpenseModal({
  ...
  const [whoPaidSelect, setWhoPaidSelect] = useState<string>(String(currentDbUserId));
  ...
  useEffect(() => {
    if (isOpen) {
      setDescription('');
      setAmount('');
      setWhoPaidSelect(String(currentDbUserId));
      setSplitType('EQUAL');
  ...
  }, [isOpen, members, currentDbUserId, defaultCurrency]);
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold text-white bg-[#5bc5a7] hover:bg-[#4bb597] transition duration-150 cursor-pointer"
      >
          ...
          <div className="relative w-full max-w-lg bg-white border border-slate-300 p-6 sm:p-8 overflow-hidden text-slate-800 my-8 max-h-[90vh] flex flex-col">
            ...
            <div className="shrink-0 mb-4 border-b border-slate-300 pb-3">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Log shared bills, define multiple contributions, and select the splitting engine.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto pr-1 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex bg-slate-100 p-0.5 border border-slate-300">
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-bold transition-all cursor-pointer ${splitType === 'EQUAL' ? 'bg-white text-slate-900 border border-slate-350' : 'text-slate-500'}`}
                    >
                      Equal
                    </button>
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-bold transition-all cursor-pointer ${splitType === 'UNEQUAL' ? 'bg-white text-slate-900 border border-slate-350' : 'text-slate-500'}`}
                    >
                      Exact
                    </button>
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-bold transition-all cursor-pointer ${splitType === 'PERCENTAGE' ? 'bg-white text-slate-900 border border-slate-350' : 'text-slate-500'}`}
                    >
                      %
                    </button>
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-bold transition-all cursor-pointer ${splitType === 'SHARE' ? 'bg-white text-slate-900 border border-slate-350' : 'text-slate-500'}`}
                    >
                      Shares
                    </button>
                  </div>
              ...
              <div className="border border-slate-300 bg-slate-50/50 p-4 max-h-56 overflow-y-auto space-y-3">
                ...
                {splitType === 'EQUAL' && (
                  <div className="text-xs text-slate-605 text-slate-600 leading-relaxed bg-white border border-slate-300 p-3">
                      ...
                      <div key={m.id} className="flex items-center justify-between gap-4 bg-white p-2 border border-slate-300">
                        ...
                        <div className="relative w-28 shrink-0">
                          <input
                            ...
                            placeholder="0.00"
                            className="block w-full pr-6 pl-2 py-1 text-xs text-right border border-slate-300 bg-white text-slate-800"
                          />
